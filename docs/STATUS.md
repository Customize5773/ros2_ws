...
| **M3** — Sensor & persepsi | 🧪 | `depth_publisher`, `qr_detector` (+`qr_offset`), bridge `camera_info` **kode selesai**. Keterbacaan QR di render kamera & isi `camera_info` **tertunda verifikasi runtime**. Kalibrasi ke kamera fisik = **OPEN** (gap hardware). |
