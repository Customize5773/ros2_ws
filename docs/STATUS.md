...
| **M5** — Manipulator | 🧪 | Node `gripper_controller` + `gripper_logic` aktif; body gripper di URDF **dipasang di muka depan ROV** (joint `0.18 0 0`, body 0.10×0.10×0.06 m, massa 0.12 kg) menjorok ke depan (+X). Integrasi `_grip` ke `mission_fsm` **dipulihkan** (PR #14 merge menghapusnya, sekarang di-cherry-pick kembali dari `4fbc6f8`). **Grasp fisik belum diuji di sim/kolam.** |
| **M6** — Autonomy (FSM misi) | 🧪 | `mission_fsm` memiliki integrasi lengkap gripper (`_grip`) + visual servo PD `APPROACH_HOOK` (`hook_logic.hook_servo`) setelah dipulihkan dari regresi merge. `done_hooks` loop DIVE untuk <4 hook, heading `APPROACH_HOOK` di-hold ke wall. QR fixes (`scan_depth=0.46`, `t_scan=60`, `start_wall`) tetap terjaga. Tuning gain/timeout & sim run penuh **tertunda verifikasi runtime**. |
