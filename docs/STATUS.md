...
| **M5** — Manipulator | 🧪 | Node `gripper_controller` + `gripper_logic` aktif; integrasi `_grip` ke `mission_fsm` **dipulihkan** (PR #14 merge menghapusnya, sekarang di-cherry-pick kembali dari `4fbc6f8`). **Grasp fisik belum diuji di sim/kolam.** |
