...
| **M5** — Manipulator | 🧪 | Node `gripper_controller` + `gripper_logic` aktif; body gripper di URDF **diperbesar** (box 0.08×0.08×0.10 m, massa 0.12 kg) agar terlihat di Gazebo. Integrasi `_grip` ke `mission_fsm` **dipulihkan** (PR #14 merge menghapusnya, sekarang di-cherry-pick kembali dari `4fbc6f8`). **Grasp fisik belum diuji di sim/kolam.** |
