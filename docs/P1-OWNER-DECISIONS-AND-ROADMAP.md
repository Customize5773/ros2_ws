...
| **R-8** | Sim terlalu ideal: nol noise, latency, dropout | ✅ **Smoke runtime host 2026-08-15.** Dengan `camera_dropout:=true`, `camera_drop_prob:=0.35`, seed `123`, dan `tether_latency_ms:=250`, Gazebo, ros_gz_bridge, injector kamera, detector, allocator, dan `gui_bridge` berhasil hidup. Injector melaporkan `dropout=True drop_prob=0.350`; frame diterima oleh `qr_detector`/`hook_detector`. Smoke berhenti normal oleh `timeout 30s`; timestamp UDP end-to-end untuk delay aktual 250 ms masih belum diukur. |
...
| **R-10** | Margin gerbang fisik attach tinggal **5–7 mm**: `alt_gap` terukur 0.073/0.074/0.075 m vs `max_alt_gap=0.08` (3/3 run) | Bukan kegagalan, tapi rapuh. Sebabnya kriteria keluar DESCEND `depth >= grab_depth - depth_tol` melepas pada ~0.66 m padahal `grab_depth=0.70` — `depth_tol=0.06` memakan hampir seluruh margin rancangan (celah rancangan 0.034 m). **Keputusan diambil 2026-08-14: opsi toleransi DESCEND lebih ketat** (bukan menaikkan `max_alt_gap`, supaya definisi "jangkauan aman cengkeram" di `gripper_logic` tak berubah). Param `descend_depth_tol` (default 0.02, terpisah dari `depth_tol` yang tetap dipakai APPROACH_QR), exit DESCEND (`mission_fsm.py`) pakai `depth >= grab_depth - descend_depth_tol`. **Battery pembanding dijalankan 2026-08-15** (`tools/p0-experiments/run_r10_descend_tol_battery.sh`, param baru `descend_depth_tol` dipaparkan lewat `hydroships_mission.launch.py`; 3 seed sama dipakai dua kali: `descend_depth_tol:=0.06` replikasi perilaku lama vs `:=0.02` default baru, `spawn_seed:=3001/3002/3003` sama persis dgn battery R-9/R-11 sebelumnya): **arah efek TERKONFIRMASI tapi bukti tak bersih, jangan tutup penuh:** **Catatan penting:** `max_alt_gap` sudah dinaikkan 0.08→0.12 di commit terpisah (alasan "account for variation during payload spawns") SEBELUM battery ini jalan — framing lama "margin 5-7mm dari 0.08" sudah usang, ambang aktual sekarang 0.12. **seed 3001**: `alt_gap` 0.033 (tol lama) → **-0.001** (tol baru) — arah sesuai hipotesis (mengecil), TAPI nilai negatif berarti dasar gripper PAS/SEDIKIT DI BAWAH bidang lantai QR, kasus tepi baru yang perlu diawasi, bukan murni "lebih aman". **seed 3002**: `alt_gap` 0.075 (tol lama, margin 0.045 ke 0.12) → **0.005** (tol baru, margin 0.115) — arah sesuai hipotesis & margin membaik jelas, TAPI run "lama" gagal bukan karena alt_gap: gerbang latch (`armed=False`) tak pernah ter-arm sama sekali (isu terpisah, mirip pola R-11), jadi alt_gap di run itu tak pernah benar-benar diuji sebagai gerbang penentu. **seed 3003**: run "lama" (`descend_depth_tol:=0.06`) **tak pernah mengirim "close" dalam window 75s** — inconclusive (bukan kegagalan gerbang, kemungkinan window kurang panjang atau GRAB belum tercapai); run "baru" (`0.02`): `alt_gap`=0.039, margin sehat (0.081) ke 0.12. **Kesimpulan:** arah perbaikan (alt_gap mengecil dgn tol lebih ketat) konsisten di 2/3 pasangan data yang bisa dibandingkan langsung, sejalan hipotesis desain. **BELUM ditutup** karena: n kecil (3 seed), 1 run inconclusive (3003-lama), 1 nilai edge-case negatif (3001-baru) yang perlu dipahami dulu (kemungkinan `descend_depth_tol=0.02` kadang overshoot sedikit melewati `grab_depth`, bukan berhenti tepat di atasnya — cek arah tanda depth vs grab_depth di `_st_descend`), dan konfound `max_alt_gap` yang sudah berubah dari nilai desain awal R-10. Log mentah: `/tmp/r10-descend-tol/R10-{before,after}-{3001,3002,3003}.log`. |
| **R-11** | *(2026-08-14, akar penyebab TERBUKTI; 2026-08-15, RESOLVED — Opsi 3)* `_st_approach_qr` (`mission_fsm.py:702-711`) hanya mengecek offset visual (`centered`) di dalam blok `if self._wall_scored:` — `_wall_scored` cuma jadi `True` kalau **huruf QR ter-decode**. Tapi offset `ex/ey` (dari deteksi kontur) adalah jalur deteksi **terpisah** dari decode huruf — dan decode huruf gagal **82–89% run** (P0-2.x, §3 tabel). Jadi tiap kali decode gagal, `centered` **tidak pernah dievaluasi**, dan `converged_now` jatuh ke `dist < approach_tol=0.06` murni — buta total terhadap seberapa jauh QR dari tengah frame. `DESCEND` cuma dapat ~3–4 detik servo pasif sebelum `depth_ok` memaksa `GRAB`, sering tak cukup mengoreksi offset besar, dan offset kerap basi (`fresh=False`) di tengah turun. **Bukan kebetulan 1 seed** — terikat langsung ke tingkat dropout decode QR yang sudah lama didokumentasikan. **Direproduksi deterministik 2026-08-14** (`R11-3002-pinned`, payload dipin `qr_letter:=C payload_x:=0.34 payload_y:=-0.35 randomize_pos:=false` + `spawn_seed:=3002`): `CONVERGEDBG: centered=False dist=0.058 approach_tol=0.060 wall_scored=False qr=- ex=+0.92 ey=+0.89` → GRAB ditolak (`x=+0.918 fresh=False`) → `GRAB timeout` → `ABORT` jujur (sama seperti run asli). Instrumentasi non-fungsional `CONVERGEDBG` ditambahkan di `_st_approach_qr` (log sekali persis saat tick konvergen — debug print periodik lama sering melewatkannya). **Opsi perbaikan (keputusan 2026-08-15: **Opsi 3 — kombinasi**):** (1) lepas cek `centered` dari gerbang `_wall_scored` — evaluasi offset visual independen dari status decode huruf (huruf tetap dipakai utk `self.wall`/m1, tapi tak lagi syarat utk cek centering); (2) gerbang re-centering visual di DESCEND→GRAB sebelum `depth_ok` me-trigger GRAB, dengan `descend_recenter_timeout` (default 5.0s) + fallback ke GRAB bila offset stale/tidak konvergen. Keduanya di `mission_fsm.py`; 116/116 test hijau; **belum diverifikasi runtime** — butuh battery replay `R11-3002-pinned` + seed lain sebelum ditutup. |
...
| **1** | Tutup blocker P0: P0-B (ukur dulu) → P0-A → P0-C | Siklus 4-hook end-to-end ≥3× berturut, seed berbeda, kontribusi vision dilaporkan terpisah | ⏳ **2/3** (`R9-3001`/`R9-3003` DONE 100/100; `R9-3002` ABORT jujur karena R-11 — lihat §5). **R-11 RESOLVED (Opsi 3, 2026-08-15)** — pending re-run `R11-3002-pinned` + seed baru |
