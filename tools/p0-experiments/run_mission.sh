#!/bin/bash
# P0-1e closed-loop DIVE regression: one full-stack mission run.
#
# The recorder attaches BEFORE the mission starts on purpose: on the corrected
# vehicle DIVE completes in ~3 s wall-clock, so attaching after launch misses
# the whole window.
#
# usage:  run_mission.sh <tag> [extra launch args...]
#   P0_DATA_DIR=/somewhere  bash tools/p0-experiments/run_mission.sh R1
...
sleep 5
python3 "$HERE/recorder.py" "$SP/$TAG.csv" 60 > "$SP/$TAG.rec.log" 2>&1 &
REC=$!
...
grep -E "FSM\]|DIVE timeout|spawn \(random" "$SP/$TAG.log" | head -6
